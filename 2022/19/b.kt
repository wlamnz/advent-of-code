...
fun main() {
    ...
    var score = 1
    bluePrints.slice(0 until 3).forEach {
        val maxConsumableOre = max(max(max(it.oreRobotCost, it.clayRobotCost), it.obsidianRobotCost.first), it.geodeRobotCost.first)
        score *= solve(it, 32, 0, 0, 0, 0, 1, 0, 0, 0, maxConsumableOre, mutableMapOf())
    }
    println(score)
}
