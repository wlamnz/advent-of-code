...
func main() {
	...
	riskLevels = expandMap(riskLevels)
...
func expandMap(riskLevels [][]int) [][]int {
	m := len(riskLevels)
	n := len(riskLevels[0])

	var newRiskLevels [][]int

	for i := 0; i < m*5; i++ {
		newRiskLevels = append(newRiskLevels, make([]int, n*5))
	}

	// Copy original map five times from top to bottom
	for r := 0; r < m; r++ {
		for c := 0; c < n; c++ {
			for i := 0; i < 5; i++ {
				v := riskLevels[r][c] + i
